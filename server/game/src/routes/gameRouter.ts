...
async function authenticateUser(ctx: RouterContext<string>, next: () => Promise<unknown>) {
  try {
...
gameRouter.put("/game/player-state/rupees", async (ctx) => {
  try {
    ...
    let updateOperation: { $inc?: Record<string, number>; $set: Record<string, unknown> };
    if (operation === "add") {
      updateOperation = { 
...
gameRouter.get("/game/bank-account", async (ctx) => {
  try {
    const userId = ctx.state.userId;
    const bankAccounts = mongodb.getCollection<BankAccountDocument>(COLLECTIONS.BANK_ACCOUNTS);
...
gameRouter.get("/game/fixed-deposits", async (ctx) => {
  try {
    const userId = ctx.state.userId;
    const fixedDeposits = mongodb.getCollection<FixedDepositDocument>(COLLECTIONS.FIXED_DEPOSITS);
    ...
    const updatedDeposits = deposits.map(deposit => {
      if (!deposit.matured && deposit.maturityDate <= currentTime) {
        return { ...deposit, matured: true, status: 'matured' as const };
      }
      return deposit;
    });
...
gameRouter.post("/game/fixed-deposits", async (ctx) => {
  try {
    ...
    if (typeof amount !== "number" || amount <= 0 || typeof duration !== "number" || duration <= 0) {
      ctx.response.status = 400;
...
gameRouter.post("/game/fixed-deposits/:id/claim", async (ctx) => {
  try {
    ...
    const interestEarned = Math.round(deposit.amount * (deposit.interestRate / 100) * durationInYears);
...
gameRouter.get("/game/stock-portfolio", async (ctx) => {
  try {
    const userId = ctx.state.userId;
    const stockPortfolios = mongodb.getCollection<StockPortfolioDocument>(COLLECTIONS.STOCK_PORTFOLIOS);
...
gameRouter.get("/game/stock-transactions", async (ctx) => {
  try {
    const userId = ctx.state.userId;
    const stockTransactions = mongodb.getCollection<StockTransactionDocument>(COLLECTIONS.STOCK_TRANSACTIONS);
...
gameRouter.post("/game/stock-portfolio/buy", async (ctx) => {
  try {
    ...
    if (!stockId || !stockName || typeof quantity !== "number" || quantity <= 0 || typeof price !== "number" || price <= 0) {
      ctx.response.status = 400;
    ...
    const existingHoldingIndex = portfolio.holdings.findIndex(h => h.symbol === stockId);
    
    if (existingHoldingIndex >= 0) {
      // Update existing holding
      const holding = portfolio.holdings[existingHoldingIndex];
      const totalShares = holding.quantity + quantity;
      const totalInvestment = (holding.averagePrice * holding.quantity) + totalCost;
      ...
      portfolio.holdings[existingHoldingIndex] = {
        ...
        gainLossPercentage: ((price * totalShares) - totalInvestment) / totalInvestment * 100
      };
    } else {
      // Add new holding
    ...
    portfolio.totalGainLoss = portfolio.totalValue - portfolio.totalInvested;
    portfolio.lastUpdated = new Date();

    // Update portfolio in database
    await stockPortfolios.updateOne(
      { userId },
      { $set: portfolio }
    );
...
gameRouter.post("/game/stock-portfolio/sell", async (ctx) => {
  try {
    ...
    if (!stockId || typeof quantity !== "number" || quantity <= 0 || typeof price !== "number" || price <= 0) {
      ctx.response.status = 400;
    ...
    // Find holding
    const holdingIndex = portfolio.holdings.findIndex(h => h.symbol === stockId);
    if (holdingIndex === -1) {
      ctx.response.status = 400;
    ...
    if (holding.quantity === quantity) {
      ...
      portfolio.holdings[holdingIndex] = {
        ...
        gainLoss: (price * remainingShares) - (holding.averagePrice * remainingShares),
        gainLossPercentage: ((price * remainingShares) - (holding.averagePrice * remainingShares)) / (holding.averagePrice * remainingShares) * 100
      };
    }
...
gameRouter.get("/game/sync", async (ctx) => {
  try {
    ...
    // Get all game data
    const [playerState, bankAccount, fixedDeposits, stockPortfolio, stockTransactions] = await Promise.all([
      mongodb.getCollection<PlayerStateDocument>(COLLECTIONS.PLAYER_STATES).findOne({ userId }),
      mongodb.getCollection<BankAccountDocument>(COLLECTIONS.BANK_ACCOUNTS).findOne({ userId }),
      mongodb.getCollection<FixedDepositDocument>(COLLECTIONS.FIXED_DEPOSITS).find({ userId }).toArray(),
      mongodb.getCollection<StockPortfolioDocument>(COLLECTIONS.STOCK_PORTFOLIOS).findOne({ userId }),
      mongodb.getCollection<StockTransactionDocument>(COLLECTIONS.STOCK_TRANSACTIONS)
        .find({ userId })
        .sort({ timestamp: -1 })
        .limit(50)
        .toArray()
