...
const TestBankDashboard: React.FC<TestBankDashboardProps> = ({ onClose }) => {
    ...
    useEffect(() => {
        const loadRealData = async () => {
            // Get username from Phaser game registry (most reliable)
            let storedUsername = "Player";
            
            if ((window as any).game && (window as any).game.registry) {
                storedUsername = (window as any).game.registry.get("username") || "Player";
            }
            
            console.log("✅ Username from game:", storedUsername);
            setUserName(storedUsername);

            // Get balance from BalanceManager
            const balance = balanceManager.getBalance();
            setCashBalance(balance.cash);
            setBankBalance(balance.bankBalance);

            // Get transactions from BalanceManager
            const allTransactions = balanceManager.getTransactions();
            setTransactions(allTransactions);

            // Get bank account details
            try {
                const accountResponse = await bankingApi.getAccount();
                console.log("🏦 Bank account response:", accountResponse);
                if (accountResponse.success && accountResponse.data) {
                    setAccountNumber(accountResponse.data.accountNumber || "DIN-000000");
                    
                    // Get username from bank account holder name
                    if (accountResponse.data.accountHolder) {
                        setUserName(accountResponse.data.accountHolder);
                        console.log("✅ Username from bank account:", accountResponse.data.accountHolder);
                    }
                    
                    if (accountResponse.data.balance !== undefined) {
                        setBankBalance(accountResponse.data.balance);
                    }
                }
            } catch (error) {
                console.error("Failed to load bank account:", error);
                setAccountNumber("DIN-000000");
            }
        };
        ...
        const unsubscribeTx = balanceManager.onNewTransaction((transaction) => {
            setTransactions(balanceManager.getTransactions());
        });
    ...
    const handleDeposit = async () => {
        ...
        if (depositAmount > cashBalance) {
            console.error("Insufficient cash balance");
        ...
        try {
            // Call banking API
            const response = await bankingApi.deposit(depositAmount);
            
            if (response.success) {
                // Update local balance
                balanceManager.updateCash(cashBalance - depositAmount);
                ...
                balanceManager.addTransaction({
                    ...
                    description: `Deposited to bank account`,
                    location: "Dhaniverse Bank"
                });
    ...
    const handleWithdraw = async () => {
        ...
        if (withdrawAmount > bankBalance) {
            console.error("Insufficient bank balance");
        ...
        try {
            // Call banking API
            const response = await bankingApi.withdraw(withdrawAmount);
            
            if (response.success) {
                // Update local balance
                balanceManager.updateCash(cashBalance + withdrawAmount);
                balanceManager.updateBankBalance(bankBalance - withdrawAmount);
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <div className="flex flex-col justify-center w-72">
                                            ...
                                            <div className="bg-neutral-900 rounded-xl p-4 flex-1 flex flex-col justify-center">
                                                ...
                                                <p className="text-gray-500 font-pixeloid text-[10px]">Available for withdrawal</p>
                                            </div>
                                            ...
                                            <div className="bg-neutral-900 rounded-xl p-4 flex-1 flex flex-col justify-center mt-2">
                                                ...
                                                <h2 className="text-white font-pixeloid text-3xl mb-1">₹{cashBalance.toLocaleString()}</h2>
                                                <p className="text-gray-500 font-pixeloid text-[10px]">Cash in hand</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-3">
                                {transactions.length > 0 ? (
                                    transactions.slice(0, 10).map(
                                        (transaction: any) => {
                                            const isPositive = transaction.type === "deposit" || transaction.type === "stock_sell" || transaction.type === "atm_deposit";
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div>
                                                        ...
                                                        <p className="text-white/30 font-pixeloid text-[9px]">
                                    ...
                                    <div className="text-center py-16">
                                        <p className="text-white/50 font-pixeloid text-2xl select-none">
                                            No transactions yet
                                        </p>
                                    </div>
                                )}
                                ...
                                {transactions.length > 0 && (
                                    <div className="text-center py-16">
                                        <p className="text-white/50 font-pixeloid text-4xl select-none">
                                            {transactions.length > 10 ? `Showing 10 of ${transactions.length} transactions` : "No more transactions!"}
                                        </p>
                                    </div>
                                )}
                            </div>
